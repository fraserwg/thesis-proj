...
\label{sec:FirstLook}
Equation~\ref{eq:PVSI1} gives a necessary but not sufficient criterion for the excitement of symmetric instability; it states that the planetary vorticity and the potential vorticity of a flow must have opposite signs. Consider now a fluid initially stable to symmetric instability (i.e. $fQ \geq 0$). For it to become symmetrically unstable we must make the quantity $fQ$ negative. There are then two distinct ways to generate symmetric instability in such a fluid: the first is to change the sign of the planetary vorticity, and the second is to change the sign of the potential vorticity.
...
\label{sec:InviscidInstabilities}
    ...
    We now consider a flow which is, initially, purely meridional, with $(u, v, w) = (0, V, 0)$, $P = \Pi$ and $b = B$. The flow is steady and in both geostrophic and hydrostatic balance. Mathematically
    \begin{subequations}
...
\label{sec:DrasticSI}
    ...
    Similarly to section~\ref{sec:InviscidInstabilities} we will now consider a flow which is, initially, purely meridional, with $(u, v, w) = (0, V, 0)$, $P = \Pi$ and $b = B$. The flow is steady, in geostrophic balance and hydrostatically balanced. Mathematically
    \begin{subequations}
    \begin{equation}
        f V = \pdv{\Pi}{x} \, ,
    \end{equation}
