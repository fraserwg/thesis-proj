...
\label{chap:2}
\begin{quote}
    \textit{In real life there is no algebra}
    \newline Audrey Horne, Twin Peaks
\end{quote}
...
\footnotetext{This is the component of the Coriolis force that acts on flows moving zonally and vertically as a result of the earth's rotation about its local meridional axis.}
...
\label{sec:InviscidInstabilities}
    ...
    \footnotetext{A meridionally symmetric flow is invariant in the meridional direction, so all terms in $\partial_y$ in its equations of motion will go to zero.}
    \begin{subequations}
...
\footnotetext{ A limitation of using neutral density to estimate the buoyancy frequency is that neutral density does not reliably preserve vertical buoyancy gradients \citep{Eden1999}. Here, we only really require a buoyancy frequency of the right order of magnitude, and this is unlikely to be the biggest source of errors in the subsequent calculations.}
...
\footnotetext{See section~\ref{sec:relation2nbc} for further details.} 
...
\label{sec:DrasticSI}
    ...
    \footnotetext{This is done to simplify the notation, however, the arguments subsequently made still hold.}
    ...
    \footnotetext{This will be shown to be a reasonable prediction in section~\ref{subsec:3DResults}}
